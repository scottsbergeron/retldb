...
TEST(ErrorTest, ErrorStringInvalid) {
    EXPECT_STREQ("Invalid error code", retldb_error_string((retldb_error_t)-1));
    EXPECT_STREQ("Invalid error code", retldb_error_string((retldb_error_t)(RETLDB_ERROR_UNKNOWN + 1)));
} 